function [V,S1,S2]= DuelSolve(A,K1,K2,M1,M2)
    ...
    for i=1:s
      ...
      if(states(i,5)==1&&child) %player one moves
        for j=1:sz
          ...
          payoff=miss*pay1(temp)*abs(Q1)+hit*Q1; %second part is zero if it's a non-shooting move
          if(payoff>=pay1(i)||payoff==Q1)
            best(i)=temp;
            pay1(i)=payoff;
            pay2(i)=miss*pay2(temp)*abs(Q1)+hit*Q2;%the other players' expected payoff must take into account their own payoff for each possibility
          endif
        endfor
      elseif(states(i,5)==2&&child) %player two moves 
        for j=1:sz
          ...
          miss=P(i,j); %chance of missing and going to next state. zero if player doesn't shoot
          hit=1-miss; %chance of hitting and ending the game
          payoff=miss*pay2(temp)*abs(Q1)+hit*Q1; %second part is zero if it's a non-shooting move
          if(payoff>=pay2(i)||payoff==Q1)
            best(i)=temp;
            pay2(i)=payoff;
            pay1(i)=miss*pay1(temp)*abs(Q1)+hit*Q2;%the other players' expected payoff must take into account their own payoff for each possibility
          endif
        endfor
      endif
    endfor
  ...
  for i=1:s
    ...
    if(temp3==K1&&temp4==K2) %at starting position everyone has max bullets
      if(temp5==1)
        ...
        V(temp1,temp2)=pay1(i);
      else
        S2(temp1,temp2)=temp;
        %V(temp1,temp2)=pay2(i); %this is equivalent to the above for zero sum
      endif
    endif
  endfor
endfunction
